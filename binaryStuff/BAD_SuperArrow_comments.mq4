...
int Gi_unused_128 = 0;
bool Gi_132 = FALSE; // EMA signal for BUY
bool Gi_136 = FALSE; // RSI signal for BUY
bool Gi_140 = FALSE; // Bears signal for BUY
bool Gi_144 = FALSE; // BB signal for BUY
bool Gi_148 = FALSE;
bool Gi_152 = FALSE; // EMA signal for SELL
bool Gi_156 = FALSE; // RSI signal for SELL
bool Gi_160 = FALSE; // Bulls signal for SELL
bool Gi_164 = FALSE; // BB signal for SELL
bool Gi_168 = FALSE;
int Gi_172 = 0;      // Flag for position
bool Gi_176 = FALSE; // Alert flag for BUY
bool Gi_180 = FALSE; // Alert flag for SELL
...
int start() {
   ...
   for (int Li_4 = 0; Li_4 <= Li_0; Li_4++) {
      ...
      Ld_132 = 0;
      Ld_140 = 0;
      for (Li_8 = Li_4; Li_8 <= Li_4 + 10; Li_8++) 
      ...
      if (Close[Li_4] > ibands_152 && Close[Li_4 + 1] >= ibands_168) {
         Gi_144 = FALSE;
         Gi_164 = TRUE;    // ~ current and previous close above top BB... SELL
      }
      if (Close[Li_4] < ibands_160 && Close[Li_4 + 1] <= ibands_176) {
         Gi_144 = TRUE;    // ~ current and previous close below bottom BB... BUY
         Gi_164 = FALSE;
      }
      if (ibullspower_60 > 0.0 && ibullspower_68 > ibullspower_60) {
         Gi_140 = FALSE;
         Gi_160 = TRUE;    // ~ bulls positive but decreasing? SELL
      }
      if (ibearspower_76 < 0.0 && ibearspower_84 < ibearspower_76) {
         Gi_140 = TRUE;    // ~ bears negative but increasing? BUY
         Gi_160 = FALSE;
      }
      if (irsi_44 > 50.0 && irsi_52 < 50.0) {
         Gi_136 = TRUE;    // ~ RSI increasing? BUY
         Gi_156 = FALSE;
      }
      if (irsi_44 < 50.0 && irsi_52 > 50.0) {
         Gi_136 = FALSE;
         Gi_156 = TRUE;    // ~ RSI decreasing? SELL
      }
      if (ima_12 > ima_20 && ima_28 < ima_36) {
         Gi_132 = TRUE;    // ~ Current fastMA above slowMA but...
         Gi_152 = FALSE;   //... previous fastMA below slowMA? BUY
      }
      if (ima_12 < ima_20 && ima_28 > ima_36) {
         Gi_132 = FALSE;  // ~ Current fastMA below slowMA but...
         Gi_152 = TRUE;   //... previous fastMA above slowMA? SELL
      }
      ...
      if (Gi_132 == TRUE && Gi_136 == TRUE && Gi_144 == TRUE && Gi_140 == TRUE && Gi_148 == TRUE && Gi_172 != 1) {
         ...
         Gi_172 = 1; // A BUY position is already open
      } else {
      // ======== Conditions for opening a SELL position ===========
         if (Gi_152 == TRUE && Gi_156 == TRUE && Gi_164 == TRUE && Gi_160 == TRUE && Gi_168 == FALSE && Gi_172 != 2) {
            ...
            Gi_172 = 2; //A SELL position is already open
         }
      }
   }
   return (0);
